...
export class TrainingsCategoriesService {
  ...
  saveCategories() {
    firebase.database().ref('/trainings/categories').set(this.categories);
  }

  getCategories() {
    firebase.database().ref('/trainings/categories').on('value', (data) => {
  ...
  getSingleCategory(id: number) {
    return new Promise(
      (resolve, reject) => {
        firebase.database().ref('/trainings/categories/' + id).once('value').then(
          (data) => {
            resolve(data.val());
          },
          (error) => {
